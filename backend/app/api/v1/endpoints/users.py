...
from app.api.v1.endpoints.auth import get_current_user, require_admin, require_backoffice, require_counter_view,UserOut
...
class AdminCreateCustomerRequest(BaseModel):
    ...
    email: EmailStr
    phone: str
    password: Optional[str] = None  # if omitted, a random one is generated and returned once
 
 
class AdminCreateCustomerResponse(UserOut):
    generated_password: Optional[str] = None  # only populated when we auto-generated one
 
 
@router.post("/", response_model=AdminCreateCustomerResponse, status_code=201)
async def create_customer(
    payload: AdminCreateCustomerRequest,
    current_user: User = Depends(require_backoffice),  # admin + manager
    db: AsyncSession = Depends(get_db),
):
    """Admin/manager-created customer (e.g. walk-in / phone order), separate from shop self-registration."""
    if (await db.execute(select(User).where(User.email == payload.email))).scalar_one_or_none():
    ...
    if payload.password:
        ...
        password = secrets.token_urlsafe(9)  # ~12 char random password
    ...
    user = User(
        ...
        is_active=True,
        is_verified=True,  # admin-created, no OTP step needed
    )
    db.add(user)
    await db.commit()
    await db.refresh(user)
 
    result = AdminCreateCustomerResponse.model_validate(user)
